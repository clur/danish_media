...
def NMFtopic(data,features):
    '''
    non negative matrix factorization topic modeling,
    This method generates top words in each topic
    -digits removed
    ...
    for ntopics in [5,10,15,20,25,30]:
        ...
        doctopic = doctopic / np.sum(doctopic,axis=1,keepdims=True) # we will scale the document-component matrix such that the component values associated with each document sum to one.
...
def stacked_plot(doctopic,ids):
    ...
    plt.xticks(ind+width/2, ids,rotation='vertical')
...
def heatmap_plot(doctopic, ids):
    N, K = doctopic.shape
    topic_labels = ['#{}'.format(k) for k in range(K)]
    plt.pcolor(doctopic, norm=None, cmap='Blues')
    plt.yticks(np.arange(doctopic.shape[0])+0.5, ids)
